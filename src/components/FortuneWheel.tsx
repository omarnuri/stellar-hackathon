...
export default function FortuneWheel({
  ...
  useEffect(() => {
    ...
    const setupScene = (
      ...
      assets: { pointer: PIXI.Texture | null; button: PIXI.Texture | null }
    ) => {
      ...
      gridG.stroke({
        ...
        alpha: GAME_CONFIG.COLORS.GRID_ALPHA,
      });
      root.addChild(gridG);
      ...
      if (rimParams.VISIBLE) {
        ...
        rimBase.arc(
          0,
          0,
          rimParams.RADIUS + rimParams.THICKNESS / 2,
          0,
          Math.PI * 2
        );
      ...
      if (assets && assets.pointer) {
        ...
        const ptrY = -(
          GAME_CONFIG.SECTORS.OUTER_RADIUS +
          GAME_CONFIG.GRAPHICS.POINTER.OFFSET_Y
        );
      ...
      if (assets && assets.button) {
        ...
        const btnY =
          GAME_CONFIG.SECTORS.OUTER_RADIUS +
          GAME_CONFIG.GRAPHICS.BUTTON.OFFSET_Y;
        ...
        mainWheelWrapper.addChild(button);
        buttonRef.current = button as unknown as PIXI.Sprite;
      }
    };
  ...
  const startSpin = () => {
    ...
    const targetRotation =
      GAME_CONFIG.ANIMATION.SPINS * Math.PI * 2 -
    ...
    gsap.to(spinLayerRef.current, {
      ...
      onUpdate: function () {
        ...
        const rawIndex = Math.floor(
          (normalizedRot + pointerOffset) / sectorAngle
        );
  ...
  const triggerTick = (velocity: number) => {
    ...
    if (pointerSpriteRef.current) {
      ...
      const duration = Math.max(
        GAME_CONFIG.TICKER.BASE_DURATION / (intensity * 2 + 0.5),
        0.04
      );
  ...
  const playProceduralClick = (intensity: number) => {
    ...
    gainNode.gain.linearRampToValueAtTime(
      volume,
      audioCtxRef.current.currentTime + 0.005
    );
